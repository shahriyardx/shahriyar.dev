...
export default function ResumePage() {
  const {
    name, phone, email, title, website,
    skills, languages, technologies, projects, experience, courses,
  } = resumeData
    ...
    <>
      <style>{`
        @media print {
          ...
          .resume-content { gap: 1.5rem !important; }
          .resume-content section { break-inside: avoid; }
          .resume-content h2 { font-size: 1.1rem !important; color: black !important; }
          .resume-content h3 { font-size: 0.95rem !important; color: black !important; }
          .resume-content p,
          .resume-content li,
          .resume-content span { color: #333 !important; }
          .resume-content .tag {
            border-color: #ddd !important; color: #333 !important;
            background: #f5f5f5 !important;
          }
          .resume-content a { color: #2563eb !important; }
        }
      `}</style>
      <div className="mx-auto max-w-4xl px-6 pt-28 pb-20 resume-page">
        ...
        <div className="flex flex-col gap-10 resume-content">
          <section>
            <h2 className="mb-3 text-lg font-semibold tracking-tight">Skills</h2>
            <div className="flex flex-wrap gap-2">
              {skills.map((s) => (
                <span key={s} className="tag border px-3 py-1 text-sm">{s}</span>
              ))}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-2">
              {languages.map((l) => (
                <span key={l} className="tag border px-3 py-1 text-sm">{l}</span>
              ))}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-2">
              {technologies.map((t) => (
                <span key={t} className="tag border px-3 py-1 text-sm">{t}</span>
              ))}
            </div>
          </section>
                ...
                <div key={p.name}>
                  ...
                  {(p.liveUrl || p.repoUrl) && (
                    <div className="mt-1.5 flex items-center gap-3 text-sm">
                      {p.liveUrl && (
                        <a href={p.liveUrl} target="_blank" rel="noopener noreferrer" className="text-muted-foreground underline underline-offset-2 transition-colors hover:text-foreground">
                          Live Site
                        </a>
                      )}
                      {p.repoUrl && (
                        <a href={p.repoUrl} target="_blank" rel="noopener noreferrer" className="text-muted-foreground underline underline-offset-2 transition-colors hover:text-foreground">
                          Repository
                        </a>
                      )}
                    </div>
